...
func (ctx *context) testWithInstance(callback func(execInterface) (rep *instance.RunResult,
	...
	if ctx.crashTitle != rep.Title {
		...
		if bias {
			...
			// When reproducing bugs, crash title maybe always changed
			// but they maybe Duplicated Bug Reports
...
func (ctx *context) saveInter(title string, pstr string) {
	...
	if err != nil {
		...
		// DONE: add extract pstr to cope with syz-prog2c
		if strings.Contains(pstr, "executing program") {
			frist := strings.Index(pstr, "\n")
			pstr = pstr[frist+1:]
		}
